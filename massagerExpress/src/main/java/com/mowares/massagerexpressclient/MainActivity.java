...
public class MainActivity extends Activity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        new PreferenceHelper(this).putDeviceToken(android_id);//done
        if (!TextUtils.isEmpty(new PreferenceHelper(this).getUserId())
                && new PreferenceHelper(this).isVerify()) {
            startActivity(new Intent(this, MainDrawerActivity.class));
            this.finish();
            return;
        } else if (!TextUtils.isEmpty(new PreferenceHelper(this).getEmail())
                && !new PreferenceHelper(this).isVerify()) {
            Intent registerIntent = new Intent(MainActivity.this,
                    RegisterActivity.class);
            registerIntent.putExtra("goToVerify", true);
