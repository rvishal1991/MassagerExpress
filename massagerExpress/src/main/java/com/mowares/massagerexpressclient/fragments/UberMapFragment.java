...
import com.mowares.massagerexpressclient.UberViewPaymentActivity;
...
import com.mowares.massagerexpressclient.adapter.VehicalSubTypeListAdapter;
import com.mowares.massagerexpressclient.adapter.VehicalTypeListAdapter;
...
import com.mowares.massagerexpressclient.utils.LocationHelper.OnLocationReceived;
...
public class UberMapFragment extends UberBaseFragment implements
    ...
    private VehicalSubTypeListAdapter subTypeAdapter;
    ...
    private TreeSet<Integer> type, subtypeid;
    ...
    private DriverListAdapter adapterDriver;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        type = new TreeSet<Integer>();
        subtypeid = new TreeSet<Integer>();
...
//		listSubType = new ArrayList<CategotySubType>();
        getVehicalTypes();
//		subTypeAdapter =new VehicalSubTypeListAdapter(activity, listType,this);
        // drawer.lock();
        listViewType.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                if (listType.get(position).isSelected) {

                    listType.get(position).isSelected = false;
                } else {

                    listType.get(position).isSelected = true;
                }
                if (pHelper.subtype()) {
                    subTypeAdapter = new VehicalSubTypeListAdapter(activity, listType.get(position).getSubTypes(), subid, UberMapFragment.this);
                    selectedType = position;
                    showSubTypeDialog();
                    subTypeAdapter.notifyDataSetChanged();
                } else {
                    Toast.makeText(getActivity(), "there is no sub type", Toast.LENGTH_LONG).show();
                }
    ...
    private void showSubTypeDialog() {
        subtype = new Dialog(getActivity(), R.style.MyDialog);
        subtype.requestWindowFeature(Window.FEATURE_NO_TITLE);
        subtype.getWindow().setBackgroundDrawable(new ColorDrawable(android.graphics.Color.TRANSPARENT));
        subtype.setContentView(R.layout.dialog_subtype);
        subtype.setCancelable(false);

        listViewSubType = (ListView) subtype.findViewById(R.id.subtypelist);
        listViewSubType.setAdapter(subTypeAdapter);
        subtype.setCancelable(false);
        subtype.findViewById(R.id.btntypeSubmit).setOnClickListener(this);
        subtype.findViewById(R.id.btnBack).setOnClickListener(this);
        subtype.show();
    }
...
//			getSelectedPosition();
    ...
    protected boolean isValidate() {
        ...
        for (int j = 0; j < listType.size(); j++) {
            if (listType.get(j).isSelected) {
                type.add(listType.get(j).getId());
                for (int i = 0; i < listType.get(j).getSubTypes().size(); i++) {
                    if (listType.get(j).getSubTypes().get(i).isSelected) {
                        subtypeid.add(listType.get(j).getSubTypes().get(i).getId());
                        Log.d("sub type id", TextUtils.join(",", type));
                        isSelectedType = true;
                    }
                }
            }
    ...
    private void pickMeUp() {
        ...
        map.put(Const.Params.TYPE, TextUtils.join(",", type));
        map.put(Const.Params.SUB_TYPE, TextUtils.join(",", subtypeid));
...
//					AndyUtils.removeCustomProgressDialog();
            ...
            case Const.ServiceCode.GET_VEHICAL_TYPES:
                if (activity.pContent.isSuccess(response)) {
                    listType.clear();
//				subTypeAdapter.notifyDataSetChanged();
                    activity.pContent.parseTypes(response, listType, selectedType);
                    pointer = listType.size();
//				VehicalType vehicle = (VehicalType) listType.get(selectedType);
//				selectedType=vehicle.getId();
...
//				}
            ...
            case Const.ServiceCode.GET_PROVIDERS_LIST:
                ...
                if (activity.pContent.isSuccess(response)) {
                    ...
                    showDriverListDialog(activity.pContent.getDriverList(response));
    ...
    private String getSelectedType() {
        TreeSet<Integer> listSelectedType = typeAdapter.getSelectedId();
//		for(int j=0;j<listSelectedType.size();j++){
//			Log.d("OOOOOOOOOOOOOOOO",j+"");
//		}
        return TextUtils.join(",", listSelectedType);

    }


    private String getSelectedSubType() {
        TreeSet<Integer> listSelectedSubType = subTypeAdapter.getSelectedId();
        for (int i = 0; i < listSelectedSubType.size(); i++) {
            Log.d("RRRRRRRRRRRRRRRRRR", i + "");
        }

//		for (int i = 0; i < subTypeAdapter.getCount(); i++) {
//			if (subTypeAdapter.checkedHolder[i]){
//			}
//			Log.d("SSSSSSSSSSSSSS",listType.get(i).getId()+"");
//
//		}
        return TextUtils.join(",", listSelectedSubType);
    }
    ...
    public void onConntected(Location location) {
        ...
        if (location != null) {
            ...
            // getAllProviders(latLang, Const.ServiceCode.GET_PROVIDERS);
    ...
    private void showDriverListDialog(ArrayList<Driver> listDriver) {
        ...
        mDialog.setContentView(R.layout.fragment_driver_list);

        mDialog.getWindow().setBackgroundDrawable(
                new ColorDrawable(android.graphics.Color.TRANSPARENT));

        this.listDriver = listDriver;
        listViewDriver = (ListView) mDialog.findViewById(R.id.listViewDriver);
        adapterDriver = new DriverListAdapter(activity, listDriver);
        listViewDriver.setAdapter(adapterDriver);
        listViewDriver.setOnItemClickListener(new OnItemClickListener() {
        ...
        mDialog.findViewById(R.id.ivBack).setOnClickListener(
                new OnClickListener() {
