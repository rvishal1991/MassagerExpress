...
import com.mowares.massagerexpressclient.parse.MultiPartRequester;
...
import com.mowares.massagerexpressclient.utils.Validation;
...
public class UberRegisterFragment extends UberBaseFragmentRegister
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        activity.setIconMenu(R.drawable.massager);
    ...
    protected boolean isValidate() {
        ...
        if (TextUtils.isEmpty(etFName.getText().toString())) {
            ...
            Log.d("66666666 ", "okokokok");
        }
        /*
         if(!Validation.isPhoneNumber(etNumber,true))
		{
			Log.d("Valid no :: ", ""+isValidPhoneNumber(etNumber.getText().toString()));
			
			msg = getString(R.string.text_enter_no);
			etNumber.requestFocus();
			Log.d("7777777777 ", "okokokok");
			
			return false;
		}*/

        String num = spCCode.getText().toString() + etNumber.getText().toString().substring(1);
    ...
    private void registerSocial(final String id, final String type) {
        ...
        //AndyUtils.showCustomProgressDialog(activity, getString(R.string.text_registering), true, null);
       // new MultiPartRequester(activity, map, Const.ServiceCode.REGISTER, this);//comment bt me
        new HttpRequester(activity,map,Const.ServiceCode.REGISTER,this);
        ...
        switch (serviceCode) {
            case Const.ServiceCode.REGISTER:
                ...
                if (pContent.isSuccessWithStoreId(response)) {
                    ...
                    activity.phelper.putEmail(etEmail.getText().toString().trim());
                    activity.phelper.putPassword(etPassword.getText().toString());
                    goToVerifyFragment();
                    ...
                    Toast.makeText(activity, getString(R.string.toast_register_failed), Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
    ...
    private void beginCrop(Uri source) {
        ...
        Uri outputUri = Uri.fromFile(new File(Environment
                ...
                .getTimeInMillis() + ".jpg")));
        new Crop(source, outputUri).asSquare().start(activity);//comment by me
    }

    private void handleCrop(int resultCode, Intent result) { //Edit by me
        if (resultCode == activity.RESULT_OK) {
