...
import com.mowares.massagerexpressclient.db.DBHelper;
import com.mowares.massagerexpressclient.maputils.PolyLineUtils;
import com.mowares.massagerexpressclient.models.ApplicationPages;
import com.mowares.massagerexpressclient.models.Bill;
...
import com.mowares.massagerexpressclient.models.DriverLocation;
import com.mowares.massagerexpressclient.models.History;
import com.mowares.massagerexpressclient.models.MyThings;
import com.mowares.massagerexpressclient.models.PTour;
import com.mowares.massagerexpressclient.models.Reffral;
import com.mowares.massagerexpressclient.models.Route;
import com.mowares.massagerexpressclient.models.Step;
import com.mowares.massagerexpressclient.models.Tour;
import com.mowares.massagerexpressclient.models.TypeInvoice;
import com.mowares.massagerexpressclient.models.User;
...
import com.mowares.massagerexpressclient.utils.ReadFiles;
...
public class ParseContent {
    ...
    private final String SUBNAME = "name";
    private final String AGE = "age";
    private final String TYPE = "type";
    private final String SUBTYPE = "sub_type";
    ...
    private final String SUBICON = "icon";
    ...
    private final String WALKER_LIST = "provider";
    ...
    public boolean isSuccessWithStoreId(String response) {
        ...
        try {
            ...
            if (jsonObject.getBoolean(KEY_SUCCESS)) {
              ...
              /*  preferenceHelper.putIsVerify(true);*/
    ...
    public boolean isSuccessWithLoginID(String response) {
        AppLog.Log(Const.TAG, response);
        if (TextUtils.isEmpty(response))
            return false;
        try {
            ...
            if (jsonObject.getBoolean(KEY_SUCCESS)) {
                ...
                preferenceHelper.putSessionToken(jsonObject
                        .getString(Const.Params.TOKEN));
                if (jsonObject.getInt(Const.Params.PROFILE_COMPLETE) == 1) {
                    preferenceHelper.putIsRegistered(true);
                } else if (jsonObject.getInt(Const.Params.PROFILE_COMPLETE) == 0) {
                    preferenceHelper.putIsRegistered(false);
                }
                preferenceHelper.putEmail(jsonObject
                        .optString(Const.Params.EMAIL));
                preferenceHelper.putLoginBy(Const.MANUAL);
                preferenceHelper.putIsVerify(true);
                preferenceHelper.putIsLoogedIn(true);
                return true;
            } else {
                AndyUtils.showToast(jsonObject.getString(KEY_ERROR), activity);
                // AndyUtils.showErrorToast(jsonObject.getInt(KEY_ERROR_CODE),
    ...
    public int checkRequestStatus(String response) {
        ...
        try {
           ...
           /* if (jsonObject.getInt("is_cancelled") == 1) {
                status = Const.IS_CANCELLED;
                return Const.IS_CANCELLED;
            }*/
            if (jsonObject.getInt(CONFIRMED_WALKER) == 0
                ...
                if (jsonObject.getInt(IS_WALKER_STARTED) == 1) {
                    status = Const.IS_WALKER_STARTED;
                }
                if (jsonObject.getInt(IS_WALKER_ARRIVED) == 1) {
                    status = Const.IS_WALKER_ARRIVED;
                }
                if (jsonObject.getInt(IS_WALK_STARTED) == 1) {
                    status = Const.IS_WALK_STARTED;
                }
                if (jsonObject.getInt(IS_COMPLETED) == 1) {
                    status = Const.IS_COMPLETED;
                }
                if (jsonObject.getInt(IS_WALKER_RATED) == 1) {
                    status = Const.IS_WALKER_RATED;
                }
            }

            AppLog.Log("status of provider", String.valueOf(status));
    ...
    public Driver getDriverDetail(String response) {
        ...
        try {
            ...
            if (jsonObjectBill != null) {
                ...
                JSONArray jsonArrayType = jsonObjectBill.getJSONArray(TYPE);
                for (int i = 0; i < jsonArrayType.length(); i++) {
                    ...
                    typeInvoice.setName(jsonObjectType.getString(NAME));
                    typeInvoice.setBasePrice(jsonObjectType.getInt(PRICE));
                    listTypeInvoice.add(typeInvoice);
                }

                bill.setListTypeInvoice(listTypeInvoice);
                AppLog.Log("listTypeInvoice", "" + bill.getListTypeInvoice());
                driver.setBill(bill);
            }
    ...
    public ArrayList<Driver> getDriverList(String response) {
        ...
        if (!TextUtils.isEmpty(response)) {
            ...
            try {
                ...
                if (object.getBoolean(KEY_SUCCESS)) {
                    JSONArray jsonArray = object.getJSONArray(WALKER_LIST);
                    if (jsonArray.length() > 0) {
                        for (int i = 0; i < jsonArray.length(); i++) {
                            AppLog.Log("list", "" + driversList);
                            JSONObject jsonDriverObj = jsonArray
                                    .getJSONObject(i);
                            Driver driver = new Driver();
                            driver.setBio(jsonDriverObj.getString(Const.Params.BIO));
                            driver.setDriverId(jsonDriverObj
                                    .getInt(Const.Params.ID));
                            driver.setFirstName(jsonDriverObj
                                    .getString(Const.Params.FIRSTNAME));
                            driver.setLastName(jsonDriverObj
                                    .getString(Const.Params.LAST_NAME));
                            driver.setPicture(jsonDriverObj
                                    .getString(Const.Params.PICTURE));
                            driver.setPhone(jsonDriverObj
                                    .getString(Const.Params.PHONE));
                            driver.setLatitude(jsonDriverObj
                                    .getDouble(Const.Params.LATITUDE));
                            driver.setLongitude(jsonDriverObj
                                    .getDouble(Const.Params.LONGITUDE));
                            driver.setRating(jsonDriverObj
                                    .getDouble(Const.Params.RATE));
                            driver.setBasePrice(jsonDriverObj.getString(Const.Params.BASE_PRICE));
                            // driver.setTotal(jsonDriverObj
                            // .getString("total_price"));

                            driversList.add(driver);
                            AppLog.Log("driverlist", "" + driversList.size());
                        }
                    }
                }
            } catch (Exception e) {
    ...
    public ArrayList<VehicalType> parseTypes(String response,
                                             ArrayList<VehicalType> list, int typePosition) {
        JSONObject jsonObject;
        try {
            jsonObject = new JSONObject(response);
            if (jsonObject.getBoolean(KEY_SUCCESS)) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    JSONArray subArray = typeJson.getJSONArray(SUBTYPE);
                    if (subArray.length() > 0) {
                        preferenceHelper.putSubtype(true);
                        ArrayList<CategotySubType> arrayList = new ArrayList<CategotySubType>();
                        for (int j = 0; j < subArray.length(); j++) {
                            CategotySubType subtype = new CategotySubType();
                            JSONObject subJson = subArray.getJSONObject(j);

                            subtype.setBasePrice(subJson.getString(BASE_PRICE));
                            subtype.setBaseDistance(subJson
                                    .getInt(BASE_DISTANCE));
                            // subtype.setUnit(subJson.getString(UNIT));
                            subtype.setId(subJson.getInt(SUBID));
                            subtype.setIcon(subJson.getString(SUBICON));
                            subtype.setName(subJson.getString(SUBNAME));
                            subtype.setPricePerUnitDistance(subJson
                                    .getDouble(PRICE_PER_UNIT_DISTANCE));
                            subtype.setPricePerUnitTime(subJson
                                    .getDouble(PRICE_PER_UNIT_TIME));
                            // subtype.setMinFare(subJson.getString(MIN_FARE));
                            subtype.setMaxSize(subJson.getString(MAX_SIZE));
                            arrayList.add(subtype);
                            type.setSubTypes(arrayList);
                        }
                    } else {
                        preferenceHelper.putSubtype(false);
                    }
                ...
                if (jsonArray.length() > 0) {
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        double distance = Double.parseDouble(object
                                .getString("main_total"));// old was distance
                        // bill.setDistance(jsonObject.getString(DISTANCE));
                        ...
                        JSONArray jsonArrayType = object
                                .getJSONArray(Const.Params.TYPE);
    ...
    public boolean isNotVerified(String response) {
        AppLog.Log(Const.TAG, response);
        if (TextUtils.isEmpty(response))
            return false;
        try {
            JSONObject jsonObject = new JSONObject(response);
            if (jsonObject.getString("error").equals("User Not Verify")) {
                return true;
            }
        } catch (JSONException e) {
            e.printStackTrace();
